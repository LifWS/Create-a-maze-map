...
int main() {
  ...
  wb_display_draw_text(ground_display, "x", width - 15, height / 2 - 15);
  wb_display_set_color(ground_display, BLUE);
  wb_display_draw_line(ground_display, width / 2, 0, width / 2, height - 1);
  wb_display_draw_text(ground_display, "z", width / 2 - 15, height - 15);
  */
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    wb_display_set_opacity(ground_display,0.05);
    wb_display_set_color(ground_display, BLUE);
    wb_display_fill_oval(ground_display, width * (translation[0] + GROUND_X / 2) / GROUND_X,
                         height * (translation[2] + GROUND_Z / 2 ) / GROUND_Z, 1.9, 1.9);
